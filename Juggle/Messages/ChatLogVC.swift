...
class ChatLogVC: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    fileprivate func sendMessage(withText text: String) {
        if self.data.1 == nil {
            ...
            let alert = okayAlert(title: "Task has beenn deleted", message: "You cannot send a message because the user has deleted this task.")
        ...
        guard let toId = self.data.0?.uid, let fromId = Auth.auth().currentUser?.uid, let taskId = self.taskId, let taskOwnerId = self.taskOwnerId else {
            self.disableAndActivate(false)
            let alert = okayAlert(title: "Unable to Send Message", message: "There was an error while trying to send your message. Please quit and try again.")
            self.present(alert, animated: true) {
                self.dismiss(animated: true, completion: nil)
            }
            return
        ...
        childRef.updateChildValues(values) { (err, _) in
            if let error = err {
                ...
                DispatchQueue.main.async {
                    self.disableAndActivate(false)
                    let alert = self.okayAlert(title: "Unable to Send Message", message: "There was an error while trying to send your message. Please quit and try again.")
                    self.present(alert, animated: true) {
                        self.dismiss(animated: true, completion: nil)
                    }
                }
