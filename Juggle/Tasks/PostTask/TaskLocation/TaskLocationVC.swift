...
class TaskLocationVC: UIViewController {
    ...
    @objc fileprivate func handleDone() {
        if isTaskOnline {
            ...
            if mapView.annotations.isEmpty {
                let alert = okayAlert(title: "No location provided", message: "Please specify the location for where your needed service will take place.")
    ...
    fileprivate func verifyPostTask() {
        ...
        if !isTaskOnline {
            ...
            guard let locationString = locationTextField.text else {
                let alert = okayAlert(title: "Invalid Credentials", message: "Please make sure that all credentials for your task are filled out correctly.")
                present(alert, animated: true, completion: nil)
        ...
        autoRef.updateChildValues(userValues) { (err, _) in
            if let error = err {
                ...
                DispatchQueue.main.async {
                    let alert = self.okayAlert(title: "Unable to post task", message: "Please try connecting to a better network or try again later.")
    ...
    func verifyCoordinates(from string: String?) {
        if let coordinateString = string, coordinateString != "" {
            ...
            getCoordinates(from: coordinateString, completionHandlerForCoordinates: { (success, coordinate) in
                if success {
                    guard let coordinate = coordinate else {
                        DispatchQueue.main.async {
                            let alert = self.okayAlert(title: "Invalid Location", message: "This is an invalid location. We could not geolocate this area.")
                            self.present(alert, animated: true, completion: nil)
            ...
            let alert = okayAlert(title: "Invalid Location", message: "Please enter a valid address.")
            present(alert, animated: true, completion: nil)
        }
    }
    ...
    fileprivate func disableAndActivate(_ bool: Bool) {
        if bool {
