...
class ChooseTaskCategoryHeaderCell: UICollectionViewCell {
    ...
    @objc fileprivate func changeTaskCategory(_ button: UIButton) {
        ...
        //        button.tintColor = UIColor.mainBlue()
        //        delegate?.didChangeCategory(to: button.titleLabel?.text ?? Constants.TaskCategories.all)
        //        self.currentCategory = button.titleLabel?.text ?? Constants.TaskCategories.all
        
        // Commented code above is shorter but less secure.
