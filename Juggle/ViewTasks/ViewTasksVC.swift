...
class ViewTasksVC: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        ...
        fetchTaskFor(category: Constants.TaskCategories.all)
    }
        ...
        databaseRef.observeSingleEvent(of: .value, with: { (dataSnapshot) in
            ...
            snapshotDictionary.forEach({ (_, values) in
                guard let value = values as? [String : Any] else {
                    self.showNoResultsFoundView(); return
                }
                value.forEach({ (key, value) in
                    guard let dictionary = value as? [String : Any] else { self.showNoResultsFoundView(); return }
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        guard let selectedTask = chosenTask else {
            let alert = okayAlert(title: "Unable to load task", message: "This task is currently unavailable. Please try again.")
